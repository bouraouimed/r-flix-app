  ...
  Stream<MovieState> mapEventToState(MovieEvent event) async* {
    if (event is FetchPopularMoviesEvent) {
      ...
      try {
        ...
        final List<RatedMovie> ratedMoviesIds =
            await _movieRepository.getRatedMoviesIds();
      ...
      try {
        final List<Movie> userRatedMovies =
            await _movieRepository.getUserRatedMovies();
        final List<RatedMovie> ratedMoviesIds =
            await _movieRepository.getRatedMoviesIds();

        final List<Genre> genres = await _movieRepository.getCategoryNames();
        yield UserRatedMoviesLoadedState(
            userRatedMovies, genres, ratedMoviesIds);
      } catch (e) {
        yield MovieErrorState('Failed to fetch movies');
      }
      ...
      try {
        final List<Movie> searchResults =
            await _movieRepository.searchMovies(event.query.toString());
        final List<RatedMovie> ratedMoviesIds =
            await _movieRepository.getRatedMoviesIds();
        ...
        final suggestions =
            searchResults.map((movie) => movie.title).toSet().toList();

        yield MovieSearchState(
            suggestions, searchResults, genres, ratedMoviesIds);
      } catch (e) {
        yield MovieErrorState('Failed to fetch movies');
      }
    }
